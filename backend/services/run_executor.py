...
from backend.db.models.run import RunResult, TestRun
...
class RunExecutor:
    ...
    async def get_session(self, run_id: str) -> RunSession:
        session = self.sessions.setdefault(run_id, RunSession())
        return session
    ...
    async def _resolve_tool_path(self, name: str) -> str:
    ...
    async def _run_process(self, run_id: str, tool_name: str):
        ...
        try:
            ...
            stdout_task = asyncio.create_task(stream_reader(process.stdout, "stdout", stdout_chunks))
            stderr_task = asyncio.create_task(stream_reader(process.stderr, "stderr", stderr_chunks))
            await process.wait()
